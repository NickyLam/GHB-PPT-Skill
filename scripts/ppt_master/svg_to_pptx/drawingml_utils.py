...
def _scale_matrix(sx: float, sy: Optional[float] = None) -> AffineMatrix:
    return (sx, 0.0, 0.0, sx if sy is None else sy, 0.0, 0.0)


def _rotate_matrix(angle_deg: float, cx: Optional[float] = None, cy: Optional[float] = None) -> AffineMatrix:
...
def parse_stop_style(style_str: str) -> tuple[str , float]:
