...
def _is_chrome_id(elem_id: Optional[str]) -> bool:
    if not elem_id:
        return False
    lower = elem_id.lower()
...
def _extract_rotate_pivot(transform_str: str) -> Optional[tuple[float, float]]:
...
def convert_g(elem: ET.Element, ctx: ConvertContext) -> Optional[ShapeResult]:
...
def convert_element(elem: ET.Element, ctx: ConvertContext) -> Optional[ShapeResult]:
...
def convert_svg_to_slide_shapes(
    ...
    trace_out: List[Dict[str, Any]]  = None,
    ...
    root = tree.getroot()
    trace_events: List[Dict[str, Any]]  = [] if trace_out is not None else None
    ...
    # Skill root is 4 levels up (scripts/ppt_master/svg_to_pptx/drawingml_converter.py);
    # icons live at <skill_root>/templates/icons (NOT scripts/templates/icons).
    icons_dir = Path(__file__).resolve().parent.parent.parent.parent / 'templates' / 'icons'
    if icons_dir.exists():
