...
def _read_env_choice(name: str, allowed: set[str]) -> Optional[str]:
...
def _read_env_int(name: str, minimum: int, maximum: int) -> Optional[int]:
    value = os.environ.get(name)
    if value is None or not value.strip():
        return None
    try:
...
def _image_generations_url(base_url: Optional[str]) -> str:
...
def _read_size_preset() -> Optional[str]:
...
def _post_image_generation(api_key: str, base_url: Optional[str], request: dict) -> dict:
    headers = {
