...
def _chart_type_with_series(chart_root: ET.Element) -> Optional[ET.Element]:
...
def _coerce_number(value: str) -> Union[int, float] | str:
    try:
        number = float(value)
    except ValueError:
        return value
...
def _cache_values(parent: Optional[ET.Element], *, numeric: bool = False) -> List[Any]:
    if parent is None:
        return []
