...
def _find_notes_master_target(entries: Dict[str, bytes]) -> Optional[str]:
...
def _create_notes_rels_xml(slide_number: int, notes_master_target: Optional[str]) -> bytes:
...
def _slide_rels_with_notes(
    ...
    *,
    slide_number: int,
    notes_text: str,
    notes_master_target: Optional[str],
