...
def _report_result(out_file: Path, media_dir: Optional[Path]) -> None:
...
def _image_size(path: Path) -> tuple[int | None, Union[int, None]]:
...
def _zip_sha256_for_target(media_hashes: Dict[str, str], target: str) -> Optional[str]:
...
def _length_to_emu(value: str) -> Optional[int]:
...
def _vml_display_size_emu(shape: Optional[ET.Element]) -> tuple[int, int]:
...
def _m_child(elem: ET.Element, name: str) -> Optional[ET.Element]:
...
def _m_pr_val(elem: ET.Element, prop: str) -> Optional[str]:
...
def _save_data_uri(data_uri: str, media_dir: Path, index: int) -> Optional[str]:
...
def _copy_local_image(src: str, base_dir: Path, media_dir: Path, index: int) -> Optional[str]:
...
def _download_remote_image(url: str, media_dir: Path, index: int) -> Optional[str]:
...
def convert_to_markdown(input_path: str, output_path: Optional[str] = None) -> str:
