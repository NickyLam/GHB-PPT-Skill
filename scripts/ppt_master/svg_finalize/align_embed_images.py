...
def _resolve_image_path(href: str, svg_dir: Path) -> Optional[Path]:
...
def _load_pil_image(img_path: Path) -> 'PILImage' :
...
def _get_href(image: ET.Element) -> Optional[str]:
...
def count_office_vector_refs_in_svg(svg_path: Optional[str, Path]) -> int:
