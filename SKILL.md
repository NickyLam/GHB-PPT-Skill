...
- 选中后，保留 GHB 正文页的左上 Logo、右上原生章节标题框和模板空白正文底版；咨询研究型文字以 `KaiTi` 为默认意图，右上框的可见右边线贴齐页面边界；构建时必须选择实际可用的 `KaiTi/STKaiti`，否则仅可落到已验证的衬线替代 `Songti SC/SimSun`，并写入 `reports/font-resolution-report.json`；标题、证据图和来源只在页眉下方安全区生成，并用 `template-section-label` 写回右上原生框；
