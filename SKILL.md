...
- 选中后，保留 GHB 正文页的左上 Logo、右上原生章节标题框和模板空白正文底版；咨询研究型文字以 `KaiTi` 为默认意图，右上框的可见右边线贴齐页面边界；构建时优先使用实际可用的 `KaiTi/STKaiti`，否则才落到已验证的衬线替代 `Songti SC/SimSun`，并写入 `reports/font-resolution-report.json`；需要**精确楷体交付**时，传入操作者本地、项目外的 `--consulting-font /path/Kaiti.ttf`，同时使用 `--embed-fonts`：正文、右上框、渲染器和最终 PPTX 都使用该文件声明的真实字体名，文件本身不会复制进项目；标题、证据图和来源只在页眉下方安全区生成，并用 `template-section-label` 写回右上原生框；
...
咨询研究型档案需要精确楷体时，保留上述命令中的 `--embed-fonts`，并追加：

```bash
  --consulting-font ~/Downloads/Kaiti.ttf
```

`--consulting-font` 仅适用于 `consulting-research-cn-v1`，并只读取操作者提供的项目外字体文件；
它不会改变默认 GHB 项目，也不会把字体源文件纳入仓库或 Skill 包。
