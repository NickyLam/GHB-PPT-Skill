...
> 用自带 **GHB 模板**生成完整演示文稿的**自包含** pi skill。封面 template-fill 保留模板首页格式、正文走 SVG 流水线生成富视觉内容、末页续接模板致谢页，最后通过 OOXML 母版注入合并，让全 deck 每一页都挂在模板母版下（背景装饰图透出）。安装本 skill + `python-pptx` 即可出 PPT，无需另装任何依赖 skill。
...
- **自包含**：ppt-master 用到的脚本已 vendor 进 `scripts/ppt_master/`，模板放 `templates/`，外部仅依赖 `python-pptx`。
- **封面字体修复**：模板封面原用楷体，自动改成微软雅黑与正文一致。

## 快速开始

```bash
pip install python-pptx          # 唯一外部依赖（仅校验用）
```
...
vendor 进来的 ppt-master 脚本（共 57 个 `.py`）：
...
> vendor 时排除了 `__pycache__`；未引入 ppt-master 的图标库/图像生成/图表模板/live preview/confirm UI 等（本流程不用图标/图片/图表，纯排版+形状+色彩）。

### 为什么 vendor（而不是运行时引用）

- **自包含**：用户只装本 skill + `python-pptx` 即可出 PPT，无需另装 ppt-master skill。满足"安装即用"的要求。
