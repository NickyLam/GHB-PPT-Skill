...
- None yet. Baseline artifacts will be written to a non-overwriting fixture or
  artifact directory after the audit and baseline design are complete.
...
- Add fixed A/B/C content fixtures plus the D OOXML count/ending/media matrix.
- Generate and preserve the `before/` baseline evidence without changing core code.
